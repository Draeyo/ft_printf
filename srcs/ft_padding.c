...
/*   Updated: 2016/08/21 14:11:04 by vlistrat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//			flag = '0';
	...
	if ((ft_strchr(lst->flag, '+') || ft_strchr(lst->flag, ' ')) && lst->width > len
		...
		lst->width--;
	i = 0;
	if (lst->prec < len && !ft_is_c(lst) && !ft_strchr(lst->flag, '-'))
		lst->prec = 0;
	if (lst->prec > lst->width)
		return (NULL);
	if (lst->prec && len > 0 && (!ft_strchr(lst->flag, '-') || lst->conv == 's'))
		lst->width -= lst->prec;
	else if ((lst->width -= len) <= 0)
		 return (NULL);
	if (ft_strchr(lst->flag, '-') && lst->conv != 's')
		lst->width -= lst->prec;
	if (!(width = ft_strnew_digit(lst->width, lst->zeroflag)))
		return (NULL);
	if (ft_ishex(lst) && ft_strchr(lst->flag, '0') && !ft_strchr(lst->flag, '-'))
	{
		lst->prec -= 2;
		return (ft_strjoin(lst->hex, ft_strnew_digit(lst->width, lst->zeroflag)));
	}
	return (width);
}
